class MusicButtons
{
  ...
  float keyWidth;
  float keyHeight;
  boolean guitarIsMuted,voiceIsMuted,heartbeatIsMuted,harmonyIsMuted,highbeatIsMuted,pianoIsMuted;
...
void GuitarButton() 
{
  ...
  {
    fill(725*255.0f/ai.bufferSize(),255,255,130);
    ellipse(x-600, y-750, buttonWidth-10, buttonHeight-10);
  }
}
...
void VoiceButton()
{
  ...
  {
    fill(880*255.0f/ai.bufferSize(),255,255,120);
    ellipse(x-750, y-450, buttonWidth-10, buttonHeight-10);
  }
}
...
void HeartbeatButton()
{
  ...
  {
    fill(957*255.0f/ai.bufferSize(),255,255,130);
    ellipse(x-600, y-150, buttonWidth-10, buttonHeight-10);
  }
}
...
void HarmonyButton()
{
  if(dist(mouseX,mouseY,x+600,y-750)<100)
  {
    fill(1000*255.0f/ai.bufferSize(),255,255,130);
    ellipse(x+600, y-750, buttonWidth-10, buttonHeight-10);
  }
}
...
void HighbeatButton()
{
  if(dist(mouseX,mouseY,x+750,y-450)<100)
  {
    fill(1100*255.0f/ai.bufferSize(),255,255,130);
    ellipse(x+750, y-450, buttonWidth-10, buttonHeight-10);
  }
}
...
void PianoButton()
{
  if(dist(mouseX,mouseY,x+600,y-150)<100)
  {
    fill(1200*255.0f/ai.bufferSize(),255,255,130);
    ellipse(x+600, y-150, buttonWidth-10, buttonHeight-10);
  }
}
...
void StopButton()
{
  if(mouseX > x+15 && mouseX < x+80 && mouseY > y-25 && mouseY < y+55)
  {
    ...
    rect( x+40, y-20, 10, 70);
    rect( x+60, y-20, 10, 70);
    if(mousePressed && heartbeatLayer.isPlaying() && guitarLayer.isPlaying() &&
    voiceLayer.isPlaying() && harmonyLayer.isPlaying() && pianoLayer.isPlaying()
     && highbeatLayer.isPlaying())
    {
     heartbeatLayer.pause();
     guitarLayer.pause();
     voiceLayer.pause();
     harmonyLayer.pause();
     pianoLayer.pause();
